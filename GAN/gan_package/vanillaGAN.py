...
class VanillaGAN_Discriminator(nn.Module):
    ...
    def __init__(self, img_shape=IMG_SHAPE):
        ...
        self.model = nn.Sequential(
            LinearBlock(int(torch.prod(torch.tensor(img_shape))), 512, activation=nn.LeakyReLU(0.2), batch_norm=True, dropout=True, p=0.5),
            LinearBlock(512, 256, activation=nn.LeakyReLU(0.2), batch_norm=True, dropout=True, p=0.5),
            LinearBlock(256, 128, activation=nn.LeakyReLU(0.2), batch_norm=True, dropout=True, p=0.5),
